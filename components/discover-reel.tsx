...
const { verticalThreshold, horizontalThreshold, axisLockThreshold, flickVelocity, settleMs, wheelThreshold } =
  REEL_GESTURE;
...
export function DiscoverReel({
  ...
  const pointerRef = useRef<{ id: number; startX: number; startY: number; startTime: number } | null>(null);
  ...
  const wishlistedIds = useMemo(() => new Set(initialWishlistedListingIds), [initialWishlistedListingIds]);
  ...
  const goTo = useCallback(
    (direction: 1 | -1) => {
      ...
      if (nextIndex < 0 || nextIndex > listings.length - 1) {
        ...
        setDrag({ x: 0, y: 0 });
        window.setTimeout(() => setIsSettling(false), reducedMotion ? 0 : settleMs);
        return;
      }
  ...
  function handlePointerDown(e: ReactPointerEvent<HTMLDivElement>) {
    ...
    pointerRef.current = { id: e.pointerId, startX: e.clientX, startY: e.clientY, startTime: performance.now() };
  }
  ...
  function handlePointerUp(e: ReactPointerEvent<HTMLDivElement>) {
    ...
    if (axis === "x") {
      ...
      const crossedRight = dx > horizontalThreshold || (dx > axisLockThreshold && velocity > flickVelocity);
    ...
    if (axis === "y") {
      ...
      const crossedNext = dy < -verticalThreshold || (dy < -axisLockThreshold && velocity < -flickVelocity);
      const crossedPrev = dy > verticalThreshold || (dy > axisLockThreshold && velocity > flickVelocity);
  ...
  return (
    <div
      ref={containerRef}
      className="relative mx-auto h-[calc(100dvh-4rem)] w-full touch-none select-none overflow-hidden bg-neutral-950 sm:my-6 sm:h-[min(calc(100dvh-8rem),844px)] sm:max-w-sm sm:rounded-3xl sm:shadow-2xl"
    ...
    >
      {listings.length === 0 ? (
        <div className="flex h-full flex-col items-center justify-center gap-3 px-6 text-center">
          ...
          <p className="text-lg font-semibold text-white">No listings to swipe through yet</p>
        ...
        <>
          {visibleOffsets.map((offset) => {
            ...
            return (
              <div
                key={listing.id}
                className={cn("absolute inset-0", isSettling && "transition-transform duration-300 ease-out")}
                style={{ transform: `translateY(${translateY}) translateX(${translateX}px) rotate(${rotate}deg)` }}
              >
                <ReelCard
                  listing={listing}
          ...
          <div className="pointer-events-none absolute left-4 top-4 z-10 rounded-full bg-black/40 px-3 py-1 text-xs font-medium text-white backdrop-blur">
        ...
        >
          {infoDialog.type === "own" && (
            <div className="flex flex-col gap-3">
              <p className="text-sm text-muted-foreground">You can&apos;t request a swap on your own listing.</p>
              <Link href={`/listings/${infoDialog.listing.id}/edit`} className="self-start">
